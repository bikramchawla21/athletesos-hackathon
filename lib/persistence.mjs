...
export function migratePersistedState(raw) {
  ...
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
    return { ok: false, code: "MEMORY_MIGRATION_FAILED", warnings: ["not_an_object"] };
  }
  ...
  if (messages.length < 1) {
    ...
    return { ok: false, code: "MEMORY_MIGRATION_FAILED", warnings };
  }

  const memoryResult = normalizeAthleteMemory(src.memory, { allowEmptyFallback: true });
  if (!memoryResult.ok) {
    return { ok: false, code: "MEMORY_MIGRATION_FAILED", warnings: memoryResult.warnings };
  }
  ...
  if (!envelope.success) {
    console.warn("MEMORY_MIGRATION_FAILED", envelope.error.issues?.slice(0, 8));
    return { ok: false, code: "MEMORY_MIGRATION_FAILED", warnings };
  }
...
export function loadPersistedState() {
  ...
  try {
    ...
    if (!result.ok) {
      console.warn("MEMORY_MIGRATION_FAILED — clearing AthleteOS storage", result.warnings);
      try {
    ...
    console.warn("MEMORY_MIGRATION_FAILED — clearing AthleteOS storage", error);
    try {
      window.localStorage.removeItem(STORAGE_KEY);
    } catch {
      // ignore
