...
/**
 * Collect evidence/citation ids already present on persisted workspace memory.
 * Historical citations remain valid across conversations in the same workspace.
 * @param {AthleteMemory | null | undefined} memory
 * @returns {Set<string>}
 */
export function collectExistingEvidenceIds(memory) {
  /** @type {Set<string>} */
  const ids = new Set();
  if (!memory) return ids;
  for (const item of memory.goals ?? []) {
    for (const id of item.sourceMessageIds ?? []) ids.add(id);
  }
  for (const item of memory.motivations ?? []) {
    for (const id of item.sourceMessageIds ?? []) ids.add(id);
  }
  for (const item of memory.challenges ?? []) {
    for (const id of item.sourceMessageIds ?? []) ids.add(id);
  }
  for (const item of memory.significantExperiences ?? []) {
    for (const id of item.sourceMessageIds ?? []) ids.add(id);
  }
  for (const item of memory.observedPatterns ?? []) {
    for (const id of item.supportingMessageIds ?? []) ids.add(id);
  }
  for (const item of memory.athleteCorrections ?? []) {
 ...
 *
 * `knownIds` = citations allowed from the current transcript.
 * `preserveIds` = citations already on existing workspace memory (prior conversations).
 * Brand-new claims must cite knownIds (or preserveIds if reusing prior evidence).
 * Existing statements keep prior evidence when the model omits/strips historical ids.
 *
 * @param {AthleteMemory} memory
 * @param {Set<string>} knownIds
 * @param {{ preserveIds?: Set<string>, previousMemory?: AthleteMemory | null }} [options]
 * @returns {AthleteMemory}
 */
export function sanitizeMemoryMessageIds(memory, knownIds, options = {}) {
  const preserveIds = options.preserveIds instanceof Set ? options.preserveIds : new Set();
  const previous = options.previousMemory ?? null;
  const allowed = (id) => knownIds.has(id) || preserveIds.has(id);

  /**
   * @param {import('./types').MemoryItem[]} items
   * @param {import('./types').MemoryItem[]} previousItems
   */
  const filterItems = (items, previousItems) => {
    const prevByStatement = new Map(
      (previousItems ?? []).map((item) => [statementKey(item.statement), item]),
    );
    /** @type {import('./types').MemoryItem[]} */
    const nextItems = [];
    for (const item of items) {
      const keptIds = (item.sourceMessageIds ?? []).filter((id) => allowed(id));
      if (keptIds.length > 0) {
        nextItems.push({ ...item, sourceMessageIds: keptIds });
        continue;
      }
      const prev = prevByStatement.get(statementKey(item.statement));
      if (prev && (prev.sourceMessageIds?.length ?? 0) > 0) {
        // Preserve historical evidence for an existing claim; do not invent new ids.
        nextItems.push(prev);
        continue;
      }
      // Brand-new claim without valid citations — drop.
    }
    return nextItems;
  };

  const filterPatterns = (items, previousItems) => {
    const prevByStatement = new Map(
      (previousItems ?? []).map((item) => [statementKey(item.statement), item]),
    );
    const nextItems = [];
    for (const item of items) {
      const keptIds = (item.supportingMessageIds ?? []).filter((id) => allowed(id));
      if (keptIds.length > 0) {
        nextItems.push({ ...item, supportingMessageIds: keptIds });
        continue;
      }
      const prev = prevByStatement.get(statementKey(item.statement));
      if (prev) {
        // Patterns loaded from DB often have empty supportingMessageIds; keep the claim.
        nextItems.push(prev);
        continue;
      }
      // New pattern with no valid support and no prior row — drop.
    }
    return nextItems;
  };
  ...
  const next = {
    ...memory,
    goals: filterItems(memory.goals, previous?.goals),
    motivations: filterItems(memory.motivations, previous?.motivations),
    challenges: filterItems(memory.challenges, previous?.challenges),
    significantExperiences: filterItems(
      memory.significantExperiences,
      previous?.significantExperiences,
    ),
    observedPatterns: filterPatterns(memory.observedPatterns, previous?.observedPatterns),
    athleteCorrections: memory.athleteCorrections.filter((item) => {
      if (allowed(item.sourceMessageId)) return true;
      return (previous?.athleteCorrections ?? []).some(
        (prev) =>
          prev.sourceMessageId === item.sourceMessageId &&
          statementKey(prev.athleteCorrection) === statementKey(item.athleteCorrection),
      );
    }),
  };
  return parseAthleteMemory(next);
