...
- Pattern explanation must use human confidence language such as “I’m beginning to see…”, “An early pattern may be…”, “This has appeared in several different moments…”, “I’m fairly confident this is worth exploring because…”, or “I don’t think we have enough context to call this a pattern yet.”
...
Lead with the single most useful thing noticed. Connect to prior memory ONLY if the report/memory clearly supports recurrence.
Respect evidence strength:
- one observation → "This came up today…"
- emerging recurrence → "This has started showing up more than once…"
- stronger support → "This is becoming a pattern…"
 ...
 */
export function getDemoSpokenInsightSynthesis(report) {
  const title = report?.pattern?.title?.trim() || "something from today’s session";
  const explanation = report?.pattern?.explanation?.trim() || "";
  const priority = report?.sharedPriority?.trim() || "";
  const parts = [
    `One thing worth keeping an eye on: ${title.replace(/\.$/, "")}.`,
  ];
  if (explanation) {
    parts.push(explanation.replace(/\.$/, "") + ".");
  }
  if (priority) {
    parts.push(`Worth watching together: ${priority.replace(/\.$/, "")}.`);
  }
...
export async function generateSpokenInsightSynthesis(report, options = {}) {
  ...
  if (!hasUsableApiKey(apiKey)) {
    return {
      ok: true,
      status: 200,
      body: { spokenSynthesis: getDemoSpokenInsightSynthesis(report), demoMode: true },
    };
  }
  ...
  try {
    if (options.modelClient?.generateSpoken) {
      const spokenSynthesis = String(
        await options.modelClient.generateSpoken({ report, memoryContext, payload }),
      ).trim();
    ...
    return {
      ...
      status: 200,
      body: {
        spokenSynthesis: getDemoSpokenInsightSynthesis(report),
        demoMode: false,
