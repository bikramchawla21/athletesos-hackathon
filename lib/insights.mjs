...
Return JSON only with these fields:
{
  ...
  "evidenceIntro": "short conversational intro (observation language unless this phenomenon already has 3+ confirmed distinct episodes across the ledger + this session)",
  ...
  "phenomenonKey": "stable_snake_case_key_for_the_underlying_phenomenon",
  "distinctOccurrences": [
    {
...
- The same single event discussed across multiple conversations = ONE occurrence (reuse the same episode label / episode identity).
...
- phenomenonKey MUST be a stable snake_case identifier for the underlying phenomenon. When the occurrence ledger lists an existing key that matches this session’s theme, REUSE that exact key. Do not invent a new key for the same phenomenon.
- distinctOccurrences should list only episodes evidenced in THIS transcript (plus clearly restated distinct historical episodes the athlete named again). Do not copy ledger episodes that the athlete did not restate.
...
- Athlete-facing longitudinal pattern language is only appropriate when the workspace ledger for this phenomenonKey plus this session’s new confident episodes reaches at least 3 distinct episodes. Otherwise frame as observation / candidate signal.
...
- If combined occurrence maturity is still under 3 (or ambiguous), frame findings as observations — never as lasting patterns, tendencies, or “you often…”.
...
- Includes phenomenonKey (stable snake_case; reuse ledger keys when the same phenomenon recurs)
...
export const reflectionReportSchema = z.object({
  ...
  phenomenonKey: z.string().max(80).optional().default(""),
 ...
 * @param {string} [occurrenceLedgerContext]
 */
export function formatMemoryContextForInsights(memory, occurrenceLedgerContext = "") {
  const lines = [];
  if (memory) {
    lines.push(
  ...
  if (occurrenceLedgerContext) {
    if (lines.length) lines.push("");
    lines.push(occurrenceLedgerContext);
  }
  return lines.join("\n");
}
...
export async function generateInsights(messages, options = {}) {
  ...
  const memoryContext = formatMemoryContextForInsights(
    options.memory,
    options.occurrenceLedgerContext || "",
  );
...
Lead with the single most useful thing noticed. Connect to prior memory ONLY if the report/memory clearly supports recurrence across distinct real-world occurrences.
Respect evidence strength and the maturity instruction provided with the request:
- fewer than two confident distinct occurrences → observation only ("Today…" / "From what you described today…"). Never call it a pattern.
- two distinct occurrences → cautious recurrence only. Never call it an established pattern.
- three or more distinct real-world occurrences → emerging pattern language is allowed carefully.
