...
/**
 * Additive instructions for the voice PWA only.
 * Default typed/web discovery keeps DISCOVERY_INSTRUCTIONS unchanged.
 */
export const VOICE_PWA_INSTRUCTIONS = `
VOICE PWA MODE (spoken turn-taking; do not change typed-web defaults):
- Reply in 1–2 short conversational sentences, typically speakable in about 5–15 seconds.
- Prefer one precise follow-up question; at most one extra short follow-up when clearly useful.
- The athlete should talk more than AthleteOS.
- No long summaries during normal turn-taking.
- No therapy-style mirroring, generic encouragement, motivational speech, or overexplaining.
- Do not claim a lasting pattern, tendency, or “you often…” from one session or from repeated wording about one event. Longitudinal patterns require at least three distinct real-world occurrences of the same phenomenon (not three conversations).
- Do not use pattern checks during ordinary question-by-question discovery.
- Still honor discovery hard bans (no “I notice…”, paraphrase openers, praise, etc.).
- After roughly 2–4 substantive athlete answers (enough story for a careful reflection), ask exactly:
  “Anything else from today?”
  Ask this at most once. Do not ask it after every turn. Do not ask it on the first reply.
- When the athlete clearly says they are done / nothing else, keep any reply extremely short; the app will finalize the session.
`;

export const VOICE_PWA_CHAT_REPAIR_INSTRUCTIONS = `
VOICE PWA MODE repair: 1–2 short spoken-friendly sentences with at most one primary question.
No long summaries, mirroring, cheerleading, or premature pattern claims.
`;

export const VOICE_PWA_MAX_OUTPUT_TOKENS = 120;
  ...
  return {
    ...
    async generateReply({ messages, memory = null, repair = false }) {
      ...
      try {
        ...
        let baseInstructions = repair
          ? `${DISCOVERY_INSTRUCTIONS}\n\n${CHAT_REPAIR_INSTRUCTIONS}`
          : DISCOVERY_INSTRUCTIONS;
        if (voicePwa) {
          baseInstructions = repair
            ? `${baseInstructions}\n\n${VOICE_PWA_INSTRUCTIONS}\n\n${VOICE_PWA_CHAT_REPAIR_INSTRUCTIONS}`
            : `${baseInstructions}\n\n${VOICE_PWA_INSTRUCTIONS}`;
        }
        ...
        if (voicePwa) {
          payload.max_output_tokens = VOICE_PWA_MAX_OUTPUT_TOKENS;
        }
...
/**
 * @param {Message[]} messages
 * @param {{ apiKey?: string, memory?: AthleteMemory | null, client?: string | null, modelClient?: { generateReply: Function } }} [options]
 */
export async function generateChatReply(messages, options = {}) {
  ...
  const memory = options.memory ?? null;
  const clientMode = options.client ?? null;
  const apiKey = "apiKey" in options ? options.apiKey : process.env.OPENAI_API_KEY;
  if (!hasUsableApiKey(apiKey)) {
    return {
      ok: true,
  ...
  const modelClient =
    options.modelClient ?? createOpenAIChatClient(apiKey, { client: clientMode });
